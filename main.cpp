...
int main()
{
	...
	double *X = new(nothrow) double[2 * n + 1];                        //Array that will store the values of sigma(xi),sigma(xi^2),sigma(xi^3)....sigma(xi^2n)
	for (i = 0; i<2 * n + 1; i++)
	{
		X[i] = 0;
		for (j = 0; j<N; j++)
	...
	for (int cc = 0; cc < n + 1; ++cc)
		B[cc] = new double[n + 2];
	//double** B = new(nothrow) double[n + 1][n + 2];

	double *a = new(nothrow) double[n + 1];            //B is the Normal matrix(augmented) that will store the equations, 'a' is for value of the final coefficients
	for (i = 0; i <= n; i++)
		for (j = 0; j <= n; j++)
